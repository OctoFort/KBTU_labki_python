...
# login
login = False
inputBox = pygame.Rect(210, 200, 140, 32)
inputText = ""
...
while running:
    ...
    while not login:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if inputText:
                        player = database_control.player()
                        player.getUserName(inputText)
                        player.addUserName()
                        score = int(player.userScore)
                        level = int(score // 4)
                        head.grow = True
                        head.limit = score
                        login = True
        ...
        screen.fill("white")
        txt_surface = font.render(inputText, True, "black")
