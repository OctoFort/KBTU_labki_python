...
class Validator:
    def __init__(
        self,
        command,
        commands=["add", "edit", "delete", "show", "upload", "index", "index_limit"],
    ):
        self.patternNumber = "\+77[047]\d{8}$"
        self.patternNumberP = "\+7\d.*@$"
        self.patternId = "\d+$"
    ...
    def isNumberP(self, string):
        return bool(re.match(self.patternNumberP, string))
    ...
    def validate(self):
        ...
        # index_limit
        if (self.command in self.commands[6]) and len(self.arguments) < 3:
            self.error = "Limit and offser are required"
            return self
        if (self.command in self.commands[6]) and len(self.arguments) < 4:
            self.error = "Third argument is required"
            return self
        ...
        if (self.command in self.commands[3]) and not (
            ...
            or self.isNumberP(self.arguments[2])
        ):
