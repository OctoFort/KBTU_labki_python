...
class Contact:
    ...
    def save(self):
        query = f"""INSERT INTO {self.db.table} (name, phone) VALUES (%s, %s)
    ...
    @classmethod
    def all_limit(cls, limit, offset=0):
        results = cls.db.execute(
            f"SELECT * FROM {cls.db.table} LIMIT %s OFFSET %s",
    ...
    def findByNumber(cls, string):
        results = cls.db.execute(
            f"SELECT * FROM {cls.db.table} WHERE phone LIKE %s", (f"%{string}%",)
        )
        return cls.convert(results)
